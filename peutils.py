...
class SignatureDatabase:
    ...
    def __init__(self, filename=None, data=None):
        ...
        self.parse_sig = \
            re.compile('\[(.*?)\]\s+?signature\s*=\s*(.*?)(\s+\?\?)*\s*ep_only\s*=\s*(\w+)(?:\s*section_start_only\s*=\s*(\w+)|)'
                       , re.S)
    ...
    def generate_section_signatures(
        self,
        pe,
        name,
        sig_length=512,
        ...
        for (idx, section) in enumerate(pe.sections):
            ...
            sig_name = '%s Section(%d/%d,%s)' % (name, idx + 1,
                    len(pe.sections), """""".join([c for c in
                    section.Name if c in string.printable]))

            section_signatures.append(self.__generate_signature(
                pe,
                offset,
                sig_name,
                ep_only=False,
    ...
    def generate_ep_signature(
        ...
        return self.__generate_signature(pe, offset, name,
                ep_only=True, sig_length=sig_length)
...
''' \
    ...
    def __match(
        ...
        if section_start_only is True:
            ...
            scan_addresses = [section.PointerToRawData for section in
                              pe.sections]
        ...
        for idx in scan_addresses:
            result = self.__match_signature_tree(signatures,
    ...
    def __match_signature_tree(
        ...
        for (idx, byte) in enumerate([ord(b) for b in data]):
            ...
            if match.has_key('??'):
                ...
                if data_remaining:
                    matched_names.extend(self.__match_signature_tree(match_tree_alternate,
                            data_remaining, idx + depth + 1))
    ...
    def __load(self, filename=None, data=None):
        ...
        for (packer_name, signature, superfluous_wildcards, ep_only,
             section_start_only) in matches:
    ...
    if hasattr(pe, 'DIRECTORY_ENTRY_BASERELOC'):
        for base_reloc in pe.DIRECTORY_ENTRY_BASERELOC:
            ...
            for reloc in base_reloc.entries:
                ...
                if last_reloc_rva is not None and last_reloc_rva \
                    <= reloc.rva <= last_reloc_rva + 4:
                    sequential_relocs += 1
