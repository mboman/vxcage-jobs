...
__version__ = '1.2.10-%d' % int(__revision__[0x15:-2])
...
MAX_STRING_LENGTH = 1048576  # 2^20
...
directory_entry_types = [  # Architecture on non-x86 platforms
    ...
    ('IMAGE_DIRECTORY_ENTRY_COPYRIGHT', 0x07),
    ...
    ('IMAGE_DIRECTORY_ENTRY_RESERVED', 0x0f),
    ]
...
section_characteristics = [  # reserved
                             # reserved
                             # reserved
                             # reserved
                             # reserved
    ...
    ('IMAGE_SCN_LNK_OVER', 1024),
    ('IMAGE_SCN_LNK_REMOVE', 2048),
    ('IMAGE_SCN_LNK_COMDAT', 4096),
    ('IMAGE_SCN_MEM_PROTECTED', 16384),
...
debug_types = [
    ...
    ('IMAGE_DEBUG_TYPE_FPO', 0x03),
    ('IMAGE_DEBUG_TYPE_MISC', 4),
    ('IMAGE_DEBUG_TYPE_EXCEPTION', 0x05),
    ('IMAGE_DEBUG_TYPE_FIXUP', 0x06),
    ('IMAGE_DEBUG_TYPE_OMAP_TO_SRC', 0x07),
    ('IMAGE_DEBUG_TYPE_OMAP_FROM_SRC', 8),
    ('IMAGE_DEBUG_TYPE_BORLAND', 0x09),
    ('IMAGE_DEBUG_TYPE_RESERVED10', 0x0a),
    ('IMAGE_DEBUG_TYPE_CLSID', 0x0b),
    ]
...
subsystem_types = [
    ...
    ('IMAGE_SUBSYSTEM_WINDOWS_CE_GUI', 0x09),
...
SUBSYSTEM_TYPE = dict([(e[1], e[0]) for e in subsystem_types]
                      + subsystem_types)
...
MACHINE_TYPE = dict([(e[1], e[0]) for e in machine_types]
                    + machine_types)
...
relocation_types = [
    ...
    ('IMAGE_REL_BASED_MIPS_JMPADDR', 0x05),
    ('IMAGE_REL_BASED_SECTION', 0x06),
    ('IMAGE_REL_BASED_REL', 0x07),
    ('IMAGE_REL_BASED_MIPS_JMPADDR16', 0x09),
    ('IMAGE_REL_BASED_IA64_IMM64', 0x09),
    ('IMAGE_REL_BASED_DIR64', 0x0a),
    ('IMAGE_REL_BASED_HIGH3ADJ', 0x0b),
    ]

RELOCATION_TYPE = dict([(e[1], e[0]) for e in relocation_types]
                       + relocation_types)

dll_characteristics = [  # reserved
                         # reserved
                         # reserved
                         # reserved
...
RESOURCE_TYPE = dict([(e[1], e[0]) for e in resource_type]
                     + resource_type)
...
def retrieve_flags(flag_dict, flag_filter):
    ...
    return [(f[0], f[1]) for f in flag_dict.items() if isinstance(f[0],
            str) and f[0].startswith(flag_filter)]
...
class Structure:
    ...
    def sizeof_type(self, t):
        ...
        if t[0] in string.digits:
            ...
            count = int("""""".join([d for d in t if d
                        in string.digits]))
            ...
            if ',' in elm:
                ...
                for elm_name in elm_names:
                    if elm_name in self.__keys__:
                        search_list = [x[:len(elm_name)] for x in
                                self.__keys__]
    ...
    def __unpack__(self, data):
        ...
        self.__unpacked_data_elms__ = struct.unpack(self.__format__,
                data)
    ...
    def __repr__(self):
        return '<Structure: %s>' % ' '.join([' '.join(s.split())
                for s in self.dump()])
        ...
        for keys in self.__keys__:
            for key in keys:
                ...
                if isinstance(val, int) or isinstance(val, long):
                    ...
                    val = """""".join(filter(lambda c: c != '\0',
                            str(val)))

                dump_dict[key] = \
                    {'FileOffset': self.__field_offsets__[key] \
                     + self.__file_offset__,
...
class SectionStructure(Structure):
    ...
    def __setattr__(self, name, val):
        ...
        if name == 'Characteristics':
            ...
            if val:
                self.__dict__['Characteristics'] |= \
                    SECTION_CHARACTERISTICS[name]
            else:
                self.__dict__['Characteristics'] ^= \
                    SECTION_CHARACTERISTICS[name]
    ...
    def contains_offset(self, offset):
        ...
        return self.pe.adjust_FileAlignment(self.PointerToRawData,
                self.pe.OPTIONAL_HEADER.FileAlignment) <= offset \
            < self.pe.adjust_FileAlignment(self.PointerToRawData,
                self.pe.OPTIONAL_HEADER.FileAlignment) \
            + self.SizeOfRawData
    ...
    def contains_rva(self, rva):
        ...
        if self.next_section_virtual_address is not None \
            ...
            and VirtualAddress_adj + size \
            > self.next_section_virtual_address:
            size = self.next_section_virtual_address \
                - VirtualAddress_adj
...
class ImportData(DataContainer):
    ...
    def __setattr__(self, name, val):
        ...
        if hasattr(self, 'ordinal') and hasattr(self, 'bound') \
            ...
            if name == 'ordinal':
                ...
                self.struct_table.AddressOfData = \
                    self.struct_table.Ordinal
                self.struct_table.Function = self.struct_table.Ordinal
                self.struct_table.ForwarderString = \
                    self.struct_table.Ordinal
            elif name == 'bound':
                if self.struct_iat is not None:
                    self.struct_iat.AddressOfData = val
                    self.struct_iat.AddressOfData = \
                        self.struct_iat.AddressOfData
                    self.struct_iat.Function = \
                        self.struct_iat.AddressOfData
                    self.struct_iat.ForwarderString = \
                        self.struct_iat.AddressOfData
            elif name == 'address':
                self.struct_table.AddressOfData = val
                self.struct_table.Ordinal = \
                    self.struct_table.AddressOfData
                self.struct_table.Function = \
                    self.struct_table.AddressOfData
                self.struct_table.ForwarderString = \
                    self.struct_table.AddressOfData
            elif name == 'name':
                ...
                if self.name_offset:
                    ...
                    name_rva = \
                        self.pe.get_rva_from_offset(self.name_offset)
                    self.pe.set_dword_at_offset(self.ordinal_offset, 0
                            << 0x1f | name_rva)
...
class ExportData(DataContainer):
    ...
    def __setattr__(self, name, val):
        ...
        if hasattr(self, 'ordinal') and hasattr(self, 'address') \
            and hasattr(self, 'forwarder') and hasattr(self, 'name'):
...
class RelocationData(DataContainer):
    ...
    def __setattr__(self, name, val):
        ...
        if hasattr(self, 'struct'):
            ...
            if name == 'type':
                word = val << 0x0c | word & 4095
            elif name == 'rva':
                offset = val - self.base_rva
                if offset < 0:
                    offset = 0
                word = word & 0xf000 | offset & 4095
...
allowed_filename = string.lowercase + string.uppercase + string.digits \
    + "!#$%&'()-@^_`{}~+,.;=[]" + """""".join([chr(i) for i in
        range(128, 256)])
...
allowed_function_name = string.lowercase + string.uppercase \
    + string.digits + '_?@$()'
...
class PE:
    ...
    __StringTable_format__ = ('StringTable', ('H,Length',
                              'H,ValueLength', 'H,Type'))

    __String_format__ = ('String', ('H,Length', 'H,ValueLength',
                         'H,Type'))
    ...
    __IMAGE_BOUND_IMPORT_DESCRIPTOR_format__ = \
        ('IMAGE_BOUND_IMPORT_DESCRIPTOR', ('I,TimeDateStamp',
         'H,OffsetModuleName', 'H,NumberOfModuleForwarderRefs'))
    ...
    def close(self):
        if self.__from_file is True and hasattr(self, '__data__') \
            and (isinstance(mmap.mmap, type)
                 and isinstance(self.__data__, mmap.mmap) or 'mmap.mmap'
                  in repr(type(self.__data__))):
            self.__data__.close()
    ...
    def __unpack_data__(
        ...
        try:
            ...
            self.__warnings.append('Corrupt header "%s" at file offset %d. Exception: %s'
                                    % (format[0], file_offset,
                                   str(err)))
            return None
    ...
    def __parse__(
        ...
        if fname:
            ...
            try:
                ...
                if hasattr(mmap, 'MAP_PRIVATE'):
                    ...
                    self.__data__ = mmap.mmap(self.fileno, 0,
                            mmap.MAP_PRIVATE)
                else:
        ...
        self.DOS_HEADER = \
            self.__unpack_data__(self.__IMAGE_DOS_HEADER_format__,
        ...
        if not self.DOS_HEADER or self.DOS_HEADER.e_magic \
            != IMAGE_DOS_SIGNATURE:
        ...
        self.NT_HEADERS = \
            self.__unpack_data__(self.__IMAGE_NT_HEADERS_format__,
                                 self.__data__[nt_headers_offset:nt_headers_offset
                                 + 8], file_offset=nt_headers_offset)
        ...
        if 65535 & self.NT_HEADERS.Signature == IMAGE_TE_SIGNATURE:
        ...
        self.FILE_HEADER = \
            self.__unpack_data__(self.__IMAGE_FILE_HEADER_format__,
                                 self.__data__[nt_headers_offset
                                 + 4:nt_headers_offset + 4 + 32],
                                 file_offset=nt_headers_offset + 4)
        image_flags = retrieve_flags(IMAGE_CHARACTERISTICS,
                'IMAGE_FILE_')
        ...
        optional_header_offset = nt_headers_offset + 4 \
        ...
        self.OPTIONAL_HEADER = \
            self.__unpack_data__(self.__IMAGE_OPTIONAL_HEADER_format__,
                                 self.__data__[optional_header_offset:optional_header_offset
                                 + 256],
                                 file_offset=optional_header_offset)  # Read up to 256 bytes to allow creating a copy of too much data
        ...
        MINIMUM_VALID_OPTIONAL_HEADER_RAW_SIZE = 0x45

        if self.OPTIONAL_HEADER is None \
            and len(self.__data__[optional_header_offset:optional_header_offset
                    + 512]) >= MINIMUM_VALID_OPTIONAL_HEADER_RAW_SIZE:
        ...
        if self.OPTIONAL_HEADER is not None:
            ...
            if self.OPTIONAL_HEADER.Magic == OPTIONAL_HEADER_MAGIC_PE:
            ...
            elif self.OPTIONAL_HEADER.Magic \
                == OPTIONAL_HEADER_MAGIC_PE_PLUS:

                self.PE_TYPE = OPTIONAL_HEADER_MAGIC_PE_PLUS

                self.OPTIONAL_HEADER = \
                    self.__unpack_data__(self.__IMAGE_OPTIONAL_HEADER64_format__,
                        self.__data__[optional_header_offset:optional_header_offset
                        + 512], file_offset=optional_header_offset)
                ...
                MINIMUM_VALID_OPTIONAL_HEADER_RAW_SIZE = 0x45 + 4
        ...
        dll_characteristics_flags = retrieve_flags(DLL_CHARACTERISTICS,
        ...
        if self.OPTIONAL_HEADER.AddressOfEntryPoint \
        ...
        for i in xrange(int(2147483647L
                        & self.OPTIONAL_HEADER.NumberOfRvaAndSizes)):
            ...
            dir_entry = \
                self.__unpack_data__(self.__IMAGE_DATA_DIRECTORY_format__,
        ...
        rawDataPointers = \
            [self.adjust_FileAlignment(s.PointerToRawData,
             self.OPTIONAL_HEADER.FileAlignment) for s in self.sections
        ...
        if self.get_section_by_rva(self.OPTIONAL_HEADER.AddressOfEntryPoint) \
            is not None:
            ...
            ep_offset = \
                self.get_offset_from_rva(self.OPTIONAL_HEADER.AddressOfEntryPoint)
            if ep_offset > len(self.__data__):

                self.__warnings.append('Possibly corrupt file. AddressOfEntryPoint lies outside the file. '
                         + 'AddressOfEntryPoint: 0x%x'
                        % self.OPTIONAL_HEADER.AddressOfEntryPoint)
        else:

            self.__warnings.append('AddressOfEntryPoint lies outside the sections\' boundaries. '
                                    + 'AddressOfEntryPoint: 0x%x'
                                   % self.OPTIONAL_HEADER.AddressOfEntryPoint)
        ...
        if not fast_load:
            ...
            if rich_header:
                ...
                self.RICH_HEADER.checksum = rich_header.get('checksum',
                        None)
                self.RICH_HEADER.values = rich_header.get('values',
                        None)
            else:
    ...
    def parse_rich_header(self):
        ...
        try:
            rich_data = self.get_data(128, 128)
            if len(rich_data) != 128:
                return None
        ...
        for i in xrange(len(data) / 2):
            ...
            headervalues += [data[2 * i] ^ checksum, data[2 * i + 1]
                             ^ checksum]
        return result
    ...
    + len(entry) * 2] = l
                                        remainder = lengths[1] \
    - len(entry)
                                        start = offsets[1] + len(entry) \
    * 2
                                        end = offsets[1] + lengths[1] \
    * 2
                                        file_data[start:end] = [u'\0'] \
    * remainder * 2

        if is_bytearray_available():
            new_file_data = """""".join(chr(c) for c in file_data)
        else:
            new_file_data = """""".join([chr(ord(c)) for c in
                    file_data])
    ...
    def parse_sections(self, offset):
        ...
        self.sections = []
        MAX_SIMULTANEOUS_ERRORS = 0x03
        for i in xrange(self.FILE_HEADER.NumberOfSections):
            simultaneous_errors = 0
            section = \
                SectionStructure(self.__IMAGE_SECTION_HEADER_format__,
                                 pe=self)
            if not section:
                break
            ...
            if section_data.count('\0') == section.sizeof():
                self.__warnings.append('Invalid section %d. ' % i
            ...
            if section.SizeOfRawData + section.PointerToRawData \
                ...
                self.__warnings.append('Error parsing section %d. ' % i
            ...
            if section.Misc_VirtualSize > 0x10000000:
                simultaneous_errors += 1
                self.__warnings.append('Suspicious value found parsing section %d. '
                         % i
            ...
            if self.adjust_SectionAlignment(section.VirtualAddress,
                ...
                simultaneous_errors += 1
                self.__warnings.append('Suspicious value found parsing section %d. '
                         % i + 'VirtualAddress is beyond 0x10000000.')

            if self.OPTIONAL_HEADER.FileAlignment != 0 \
                and section.PointerToRawData \
            ...
            if section.__dict__.get('IMAGE_SCN_MEM_WRITE', False) \
                and section.__dict__.get('IMAGE_SCN_MEM_EXECUTE',
                    False):
                ...
                if section.Name == 'PAGE' and self.is_driver():
                    ...
                    self.__warnings.append('Suspicious flags set for section %d. '
                             % i
        ...
        self.sections.sort(cmp=lambda a, b: cmp(a.VirtualAddress,
                           b.VirtualAddress))
        for (idx, section) in enumerate(self.sections):
            if idx == len(self.sections) - 1:
                ...
                section.next_section_virtual_address = \
                    self.sections[idx + 1].VirtualAddress
    ...
    def parse_data_directories(self, directories=None):
        ...
        for entry in directory_parsing:
            ...
            try:
                ...
                dir_entry = \
                    self.OPTIONAL_HEADER.DATA_DIRECTORY[directory_index]
            except IndexError:
                break
            ...
            if directories is None or directory_index in directories:
                ...
                if dir_entry.VirtualAddress:
                    value = entry[1](dir_entry.VirtualAddress,
                            dir_entry.Size)
                    if value:
                        setattr(self, (entry[0])[0x06:], value)

            if directories is not None and isinstance(directories,
                    list) and entry[0] in directories:
    ...
    def parse_directory_bound_imports(self, rva, size):
        ...
        bnd_descr = \
            Structure(self.__IMAGE_BOUND_IMPORT_DESCRIPTOR_format__)
        ...
        while True:
            ...
            bnd_descr = \
                self.__unpack_data__(self.__IMAGE_BOUND_IMPORT_DESCRIPTOR_format__,
            ...
            if section is None:
                ...
                sections_after_offset = [section.PointerToRawData
                ...
                if sections_after_offset:
                    ...
                    first_section_after_offset = \
                        min(sections_after_offset)
                    section = \
                        self.get_section_by_offset(first_section_after_offset)
            ...
            if not section:
                self.__warnings.append('RVA of IMAGE_BOUND_IMPORT_DESCRIPTOR points to an invalid address: %x'
                         % rva)
                return
            ...
            for idx in \
                xrange(min(bnd_descr.NumberOfModuleForwarderRefs,
                       safety_boundary / 8)):
                ...
                name_str = self.get_string_from_data(0,
                        self.__data__[offset:offset
                        + MAX_STRING_LENGTH])
                ...
                if name_str:
                    invalid_chars = [c for c in name_str if c
                            not in string.printable]
                    if len(name_str) > 256 or len(name_str) < 4 \
                        or invalid_chars:
                        break
            ...
            name_str = self.get_string_from_data(0,
                    self.__data__[offset:offset + MAX_STRING_LENGTH])
            ...
            if not name_str:
                break
            bound_imports.append(BoundImportDescData(struct=bnd_descr,
                                 name=name_str, entries=forwarder_refs))
    ...
    def parse_directory_load_config(self, rva, size):
        ...
        try:
            ...
            self.__warnings.append('Invalid LOAD_CONFIG information. Can\'t read '
                                    + 'data at RVA: 0x%x' % rva)
    ...
    def parse_relocations_directory(self, rva, size):
        ...
        rlc_size = \
            Structure(self.__IMAGE_BASE_RELOCATION_format__).sizeof()
        end = rva + size
        ...
        while rva < end:
            ...
            try:
                rlc = \
                    self.__unpack_data__(self.__IMAGE_BASE_RELOCATION_format__,
                        self.get_data(rva, rlc_size),
                        file_offset=self.get_offset_from_rva(rva))
            except PEFormatError:
                self.__warnings.append('Invalid relocation information. Can\'t read '
                         + 'data at RVA: 0x%x' % rva)
                rlc = None
            ...
            if rlc.VirtualAddress > self.OPTIONAL_HEADER.SizeOfImage:
                self.__warnings.append('Invalid relocation information. VirtualAddress outside'
            ...
            if rlc.SizeOfBlock > self.OPTIONAL_HEADER.SizeOfImage:
                self.__warnings.append('Invalid relocation information. SizeOfBlock too large'
                         + ': %d' % rlc.SizeOfBlock)
                break

            reloc_entries = self.parse_relocations(rva + rlc_size,
                    rlc.VirtualAddress, rlc.SizeOfBlock - rlc_size)
    ...
    def parse_relocations(
        ...
        for idx in xrange(len(data) / 2):
            ...
            entry = \
                self.__unpack_data__(self.__IMAGE_BASE_RELOCATION_ENTRY_format__,
                    data[idx * 2:(idx + 1) * 2],
            ...
            reloc_type = word >> 0x0c
            reloc_offset = word & 4095
            if (reloc_offset, reloc_type) in offsets_and_type:
                self.__warnings.append('Overlapping offsets in relocation data '
            ...
            entries.append(RelocationData(struct=entry,
                           type=reloc_type, base_rva=rva,
    ...
    def parse_debug_directory(self, rva, size):
        ...
        dbg_size = \
            Structure(self.__IMAGE_DEBUG_DIRECTORY_format__).sizeof()
        ...
        for idx in xrange(size / dbg_size):
            try:
                ...
                self.__warnings.append('Invalid debug information. Can\'t read '
                         + 'data at RVA: 0x%x' % rva)
                return None

            dbg = \
                self.__unpack_data__(self.__IMAGE_DEBUG_DIRECTORY_format__,
    ...
    def parse_resources_directory(
        ...
        resource_dir = \
            self.__unpack_data__(self.__IMAGE_RESOURCE_DIRECTORY_format__,
                                 data,
                                 file_offset=self.get_offset_from_rva(rva))
        ...
        if number_of_entries > MAX_ALLOWED_ENTRIES:
            self.__warnings.append('Error parsing the resources directory. The directory contains %d entries (>%s)'
                                    % (number_of_entries,
                                   MAX_ALLOWED_ENTRIES))
            return None
        ...
        for idx in xrange(number_of_entries):
            ...
            if res is None:
                self.__warnings.append('Error parsing the resources directory, Entry %d is invalid, RVA = 0x%x. '
                         % (idx, rva))
                break
            ...
            name_is_string = (res.Name & 0x80000000L) >> 0x1f
            if not name_is_string:
                ...
                try:
                    ...
                    if last_name_begin_end and last_name_begin_end[0] \
                        < ustr_offset and last_name_begin_end[1] \
                        >= ustr_offset:

                        # Remove the previous overlapping entry as it's likely to be already corrupt data.

                        strings_to_postprocess.pop()
                        self.__warnings.append('Error parsing the resources directory, attempting to read entry name. Entry names overlap 0x%x'
                                 % ustr_offset)
                        break

                    last_name_begin_end = (ustr_offset, ustr_offset
                            + entry_name.get_pascal_16_length())

                    strings_to_postprocess.append(entry_name)
                except PEFormatError, excp:

                    self.__warnings.append("Error parsing the resources directory, attempting to read entry name. Can't read unicode string at offset 0x%x"
                             % ustr_offset)
            ...
            if res.DataIsDirectory:
                ...
                if base_rva + res.OffsetToDirectory in dirs:
                    ...
                    entry_directory = \
                        self.parse_resources_directory(base_rva
                            + res.OffsetToDirectory, size - (rva
                            - base_rva), base_rva=base_rva, level=level
                            + 1, dirs=dirs + [base_rva
                            + res.OffsetToDirectory])  # size
                        ...
                        if hasattr(resource_id, 'directory'):
                            ...
                            for resource_lang in \
                                ...
                                if resource_lang is None \
                                    ...
                                    or resource_lang.data.struct.Size \
                                    is None or resource_id.id is None:
                                    continue
                ...
                if struct:
                    entry_data = ResourceDataEntryData(struct=struct,
                            lang=res.Name & 0x3ff, sublang=res.Name
                            >> 0x0a)
            ...
            if level == 0 and res.Id == RESOURCE_TYPE['RT_VERSION']:
                ...
                try:
                    rt_version_struct = \
                        last_entry.directory.entries[0].directory.entries[0].data.struct
                except:
    ...
    def parse_resource_data_entry(self, rva):
        ...
        try:
            ...
            self.__warnings.append('Error parsing a resource directory data entry, '
                                    + 'the RVA is invalid: 0x%x' % rva)
            return None

        data_entry = \
            self.__unpack_data__(self.__IMAGE_RESOURCE_DATA_ENTRY_format__,
                                 data,
                                 file_offset=self.get_offset_from_rva(rva))
    ...
    def parse_resource_entry(self, rva):
        ...
        try:
            data = self.get_data(rva,
                                 Structure(self.__IMAGE_RESOURCE_DIRECTORY_ENTRY_format__).sizeof())
        except PEFormatError, excp:
        ...
        resource = \
            self.__unpack_data__(self.__IMAGE_RESOURCE_DIRECTORY_ENTRY_format__,
                                 data,
                                 file_offset=self.get_offset_from_rva(rva))
        ...
        resource.DataIsDirectory = (resource.OffsetToData
                                    & 0x80000000L) >> 0x1f
    ...
    def parse_version_information(self, version_struct):
        ...
        start_offset = \
            self.get_offset_from_rva(version_struct.OffsetToData)
        ...
        versioninfo_struct = \
            self.__unpack_data__(self.__VS_VERSIONINFO_format__,
                                 raw_data, file_offset=start_offset)
        ...
        try:
            ...
            self.__warnings.append('Error parsing the version information, '
        ...
        fixedfileinfo_offset = \
            self.dword_align(versioninfo_struct.sizeof() + 2
                             * (len(versioninfo_string) + 1),
                             version_struct.OffsetToData)
        fixedfileinfo_struct = \
            self.__unpack_data__(self.__VS_FIXEDFILEINFO_format__,
        ...
        stringfileinfo_offset = self.dword_align(fixedfileinfo_offset
                + fixedfileinfo_struct.sizeof(),
                version_struct.OffsetToData)
        ...
        while True:
            ...
            stringfileinfo_struct = \
                self.__unpack_data__(self.__StringFileInfo_format__,
            ...
            try:
                stringfileinfo_string = \
                    self.get_string_u_at_rva(ustr_offset)
            except PEFormatError, excp:
                self.__warnings.append('Error parsing the version information, '
            ...
            if stringfileinfo_string \
                and stringfileinfo_string.startswith(u'StringFileInfo'):

                if stringfileinfo_struct.Type in (0, 1) \
                    and stringfileinfo_struct.ValueLength == 0:

                    stringtable_offset = \
                        self.dword_align(stringfileinfo_offset
                            + stringfileinfo_struct.sizeof() + 2
                            * (len(stringfileinfo_string) + 1),
                            version_struct.OffsetToData)
                    ...
                    while True:
                        ...
                        stringtable_struct = \
                            self.__unpack_data__(self.__StringTable_format__,
                        ...
                        entry_offset = \
                            self.dword_align(stringtable_offset
                                + stringtable_struct.sizeof() + 2
                                * (len(stringtable_string) + 1),
                                version_struct.OffsetToData)

                        # Process all entries in the string table
                        #

                        while entry_offset < stringtable_offset \
                            + stringtable_struct.Length:

                            string_struct = \
                                self.__unpack_data__(self.__String_format__,
                            ...
                            ustr_offset = version_struct.OffsetToData \
                                + entry_offset + string_struct.sizeof()
                            try:
                                key = \
                                    self.get_string_u_at_rva(ustr_offset)
                                key_offset = \
                                    self.get_offset_from_rva(ustr_offset)
                            except PEFormatError, excp:
                                self.__warnings.append('Error parsing the version information, '
                            ...
                            value_offset = self.dword_align(2
                                    * (len(key) + 1) + entry_offset
                            ...
                            if string_struct.Length == 0:
                                entry_offset = stringtable_offset \
                                    + stringtable_struct.Length
                            else:
                                entry_offset = \
                                    self.dword_align(string_struct.Length
                                        + entry_offset,
                                        version_struct.OffsetToData)
        ...
        % ord(c))
                        ...
                        new_stringtable_offset = \
                            self.dword_align(stringtable_struct.Length
                                + stringtable_offset,
                                version_struct.OffsetToData)
                        ...
                        if stringtable_offset \
                            >= stringfileinfo_struct.Length:
                            break
            elif stringfileinfo_string \
                and stringfileinfo_string.startswith(u'VarFileInfo'):
            ...
            #
                ...
                if varfileinfo_struct.Type in (0, 1) \
                    and varfileinfo_struct.ValueLength == 0:

                    var_offset = self.dword_align(stringfileinfo_offset
                            + varfileinfo_struct.sizeof() + 2
                            * (len(stringfileinfo_string) + 1),
                            version_struct.OffsetToData)
                    ...
                    while True:
                        var_struct = \
                            self.__unpack_data__(self.__Var_format__,
                                raw_data[var_offset:],
                        ...
                        varword_offset = self.dword_align(2
                                * (len(var_string) + 1) + var_offset
                                + var_struct.sizeof(),
                                version_struct.OffsetToData)
                        orig_varword_offset = varword_offset

                        while varword_offset < orig_varword_offset \
                            + var_struct.ValueLength:
                            word1 = \
                                self.get_word_from_data(raw_data[varword_offset:varword_offset
                                    + 2], 0)
                            word2 = \
                                self.get_word_from_data(raw_data[varword_offset
                                    + 2:varword_offset + 4], 0)
                            varword_offset += 4

                            if isinstance(word1, (int, long)) \
                                and isinstance(word2, (int, long)):
                                var_struct.entry = \
                                    {var_string: '0x%04x 0x%04x' \
                                        % (word1, word2)}

                        var_offset = self.dword_align(var_offset
                                + var_struct.Length,
                                version_struct.OffsetToData)
            ...
            stringfileinfo_offset = \
                self.dword_align(stringfileinfo_struct.Length
                                 + stringfileinfo_offset,
                                 version_struct.OffsetToData)
            ...
            if stringfileinfo_struct.Length == 0 \
    ...
    def parse_export_directory(self, rva, size):
        ...
        try:
            export_dir = \
                self.__unpack_data__(self.__IMAGE_EXPORT_DIRECTORY_format__,
                    self.get_data(rva,
                    Structure(self.__IMAGE_EXPORT_DIRECTORY_format__).sizeof()),
                    file_offset=self.get_offset_from_rva(rva))
        except PEFormatError:
            self.__warnings.append('Error parsing export directory at RVA: 0x%x'
                                    % rva)
            return
        ...
        try:
            ...
            address_of_name_ordinals = \
                self.get_data(export_dir.AddressOfNameOrdinals,
                              min(length_until_eof(export_dir.AddressOfNameOrdinals),
                              export_dir.NumberOfNames * 4))
            address_of_functions = \
                self.get_data(export_dir.AddressOfFunctions,
        ...
        max_failed_entries_before_giving_up = 0x0a

        section = self.get_section_by_rva(export_dir.AddressOfNames)
        if not section:
            self.__warnings.append('RVA AddressOfNames in the export directory points to an invalid address: %x'
                                    % export_dir.AddressOfNames)
            return
        else:
            safety_boundary = section.VirtualAddress \
                + len(section.get_data()) - export_dir.AddressOfNames

        for i in xrange(min(export_dir.NumberOfNames, safety_boundary
                        / 4)):
            symbol_name_address = \
            ...
            try:
                symbol_name_offset = \
                    self.get_offset_from_rva(symbol_name_address)
            except PEFormatError:
                max_failed_entries_before_giving_up -= 1
                if max_failed_entries_before_giving_up <= 0:
                    break
            ...
            symbol_ordinal = \
                self.get_word_from_data(address_of_name_ordinals, i)

            if symbol_ordinal is not None and symbol_ordinal * 4 \
                < len(address_of_functions):
                symbol_address = \
                    self.get_dword_from_data(address_of_functions,
                        symbol_ordinal)
            else:
            ...
            if symbol_address >= rva and symbol_address < rva + size:
                ...
                try:
                    forwarder_offset = \
                        self.get_offset_from_rva(symbol_address)
                except PEFormatError:
                    continue
            else:
                forwarder_str = None
            ...
            exports.append(ExportData(
                ...
                ordinal_offset=self.get_offset_from_rva(export_dir.AddressOfNameOrdinals
                        + 2 * i),
                address=symbol_address,
                address_offset=self.get_offset_from_rva(export_dir.AddressOfFunctions
                        + 4 * symbol_ordinal),
                name=symbol_name,
        ...
        if not section:
            self.__warnings.append('RVA AddressOfFunctions in the export directory points to an invalid address: %x'
        ...
        safety_boundary = section.VirtualAddress \
            + len(section.get_data()) - export_dir.AddressOfFunctions

        for idx in xrange(min(export_dir.NumberOfFunctions,
                          safety_boundary / 4)):
            ...
            if not idx + export_dir.Base in ordinals:
                ...
                if symbol_address >= rva and symbol_address < rva \
                    + size:
                    forwarder_str = \
                        self.get_string_at_rva(symbol_address)
                else:
                    forwarder_str = None

                exports.append(ExportData(ordinal=export_dir.Base
                               + idx, address=symbol_address,
                               name=None, forwarder=forwarder_str))

        return ExportDirData(struct=export_dir, symbols=exports)

    def dword_align(self, offset, base):
        return (offset + base + 0x03 & 0xfffffffcL) - (base
                & 0xfffffffcL)
    ...
    def parse_delay_import_directory(self, rva, size):
        ...
        while True:
            try:
                ...
                self.__warnings.append('Error parsing the Delay import directory at RVA: 0x%x'
                         % rva)
                break

            file_offset = self.get_offset_from_rva(rva)
            import_desc = \
                self.__unpack_data__(self.__IMAGE_DELAY_IMPORT_DESCRIPTOR_format__,
            ...
            if rva > import_desc.pINT or rva > import_desc.pIAT:
                max_len = max(rva - import_desc.pINT, rva
                              - import_desc.pIAT)

            try:
                import_data = self.parse_imports(import_desc.pINT,
                        import_desc.pIAT, None, max_length=max_len)
            except PEFormatError, e:
                self.__warnings.append('Error parsing the Delay import directory. '
                         + 'Invalid import data at RVA: 0x%x (%s)'
                        % (rva, e.value))
                break
            ...
            if dll:
                ...
                import_descs.append(ImportDescData(struct=import_desc,
                                    imports=import_data, dll=dll))
    ...
    def get_imphash(self):
        ...
        for entry in self.DIRECTORY_ENTRY_IMPORT:
            ...
            for imp in entry.imports:
                ...
                impstrs.append('%s.%s' % (libname.lower(),
                               funcname.lower()))
    ...
    def parse_import_directory(self, rva, size):
        ...
        while True:
            try:
                ...
                data = self.get_data(rva,
                        Structure(self.__IMAGE_IMPORT_DESCRIPTOR_format__).sizeof())
            except PEFormatError, e:
                self.__warnings.append('Error parsing the import directory at RVA: 0x%x'
                         % rva)
                break

            file_offset = self.get_offset_from_rva(rva)
            import_desc = \
                self.__unpack_data__(self.__IMAGE_IMPORT_DESCRIPTOR_format__,
                    data, file_offset=file_offset)
            ...
            max_len = len(self.__data__) - file_offset
            if rva > import_desc.OriginalFirstThunk or rva \
                > import_desc.FirstThunk:
                max_len = max(rva - import_desc.OriginalFirstThunk, rva
                              - import_desc.FirstThunk)

            try:
                import_data = \
                    self.parse_imports(import_desc.OriginalFirstThunk,
                        import_desc.FirstThunk,
                        import_desc.ForwarderChain, max_length=max_len)
            except PEFormatError, e:
                self.__warnings.append('Error parsing the import directory. '
                         + 'Invalid Import data at RVA: 0x%x (%s)'
                        % (rva, e.value))
                break
            ...
            for symbol in imp_dll.imports:
                for suspicious_symbol in suspicious_imports:
                    if symbol and symbol.name \
                        and symbol.name.startswith(suspicious_symbol):
                        suspicious_imports_count += 1
                        break
                total_symbols += 1
        if suspicious_imports_count == len(suspicious_imports) \
            and total_symbols < 0x14:
    ...
    def parse_imports(
        ...
        if (not iat or len(iat) == 0) and (not ilt or len(ilt) == 0):
            raise PEFormatError('Invalid Import Table information. '
        ...
        for idx in xrange(len(table)):
            ...
            if table[idx].AddressOfData:
                ...
                if table[idx].AddressOfData & ordinal_flag:
                    ...
                    try:
                        hint_name_table_rva = table[idx].AddressOfData \
                            & address_mask
                        ...
                        imp_name = \
                            self.get_string_at_rva(table[idx].AddressOfData
                                + 2)
                        if not is_valid_function_name(imp_name):
                            imp_name = '*invalid*'

                        name_offset = \
                            self.get_offset_from_rva(table[idx].AddressOfData
                                + 2)
                    except PEFormatError, e:
                        pass
            ...
            imp_address = first_thunk + self.OPTIONAL_HEADER.ImageBase \
                + idx * imp_offset

            struct_iat = None
            try:

                if iat and ilt and ilt[idx].AddressOfData \
                    != iat[idx].AddressOfData:
            ...
            if imp_name != """""" and (imp_ord or imp_name):
                imported_symbols.append(ImportData(  # for bound imports if any
                    pe=self,
    ...
    def get_import_table(self, rva, max_length=None):
        ...
        MAX_ADDRESS_SPREAD = 128 * 2 ** 0x14  # 64 MB
        MAX_REPEATED_ADDRESSES = 0x0f
        repeated_address = 0
        ...
        while True and rva:
            ...
            if addresses_of_data_set_32 \
                and max(addresses_of_data_set_32) \
                - min(addresses_of_data_set_32) > MAX_ADDRESS_SPREAD:
                return []
            if addresses_of_data_set_64 \
                and max(addresses_of_data_set_64) \
            ...
            thunk_data = self.__unpack_data__(format, data,
                    file_offset=self.get_offset_from_rva(rva))
            ...
            if thunk_data.AddressOfData >= start_rva \
            ...
            if thunk_data and thunk_data.AddressOfData:
                ...
                if thunk_data.AddressOfData & ordinal_flag:
                    ...
                    if thunk_data.AddressOfData & 0x7fffffff > 65535:
                        return []
                else:
                ...
                # and if it looks like it should be an RVA
                    ...
                    if thunk_data.AddressOfData \
                        in addresses_of_data_set_32 \
                        or thunk_data.AddressOfData \
                        in addresses_of_data_set_64:
                        repeated_address += 1
    ...
    def get_memory_mapped_image(self, max_virtual_address=0x10000000,
        ...
        for section in self.sections:
            ...
            if section.Misc_VirtualSize == 0 or section.SizeOfRawData \
                == 0:
                continue
            ...
            VirtualAddress_adj = \
                self.adjust_SectionAlignment(section.VirtualAddress,
                    self.OPTIONAL_HEADER.SectionAlignment,
                    self.OPTIONAL_HEADER.FileAlignment)
    ...
    def get_rva_from_offset(self, offset):
        ...
        if not s:
            if self.sections:
                lowest_rva = \
                    min([self.adjust_SectionAlignment(s.VirtualAddress,
                        self.OPTIONAL_HEADER.SectionAlignment,
    ...
    def get_string_at_rva(self, rva):
        ...
        s = self.get_section_by_rva(rva)
        if not s:
            return self.get_string_from_data(0, self.__data__[rva:rva
                    + MAX_STRING_LENGTH])

        return self.get_string_from_data(0, s.get_data(rva,
    ...
    def get_string_u_at_rva(self, rva, max_length=2 ** 16):
        ...
        for idx in xrange(max_length):
            try:
                uchr = struct.unpack('<H', self.get_data(rva + 2 * idx,
                        2))[0]
            except struct.error:
                break
    ...
    def dump_info(self, dump=None):
        ...
        if hasattr(self, 'OPTIONAL_HEADER') and self.OPTIONAL_HEADER \
            is not None:
        ...
        for section in self.sections:
            ...
            for flag in section_flags:
                if getattr(section, flag[0]):
                    flags.append(flag[0])
            dump.add_line(', '.join(flags))
            dump.add_line('Entropy: %f (Min=0.0, Max=8.0)'
                          % section.get_entropy())
            if md5 is not None:
                dump.add_line('MD5     hash: %s'
                              % section.get_hash_md5())
            if sha1 is not None:
                dump.add_line('SHA-1   hash: %s'
                              % section.get_hash_sha1())
            if sha256 is not None:
                dump.add_line('SHA-256 hash: %s'
                              % section.get_hash_sha256())
            if sha512 is not None:
                dump.add_line('SHA-512 hash: %s'
                              % section.get_hash_sha512())
            dump.add_newline()
        ...
        def convert_char(char):
            if char in string.ascii_letters or char in string.digits \
                or char in string.punctuation or char \
        ...
        if hasattr(self, 'VS_VERSIONINFO'):
            ...
            if hasattr(self, 'FileInfo'):
                for entry in self.FileInfo:
                    ...
                    if hasattr(entry, 'StringTable'):
                        for st_entry in entry.StringTable:
                            [dump.add_line('  ' + line) for line in
                             st_entry.dump()]
                            dump.add_line('  LangID: '
                                    + st_entry.LangID)
                            dump.add_newline()
            ...
            for module in self.DIRECTORY_ENTRY_IMPORT:
                ...
                for symbol in module.imports:
                    ...
                    if symbol.import_by_ordinal is True:
                        ...
                        dump.add('%s.%s Hint[%s]' % (module.dll,
                                 symbol.name, str(symbol.hint)))
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_RESOURCE'):
            ...
            for resource_type in self.DIRECTORY_ENTRY_RESOURCE.entries:
                ...
                if hasattr(resource_type, 'directory'):
                    ...
                    dump.add_lines(resource_type.directory.struct.dump(),
                                   4)

                    for resource_id in resource_type.directory.entries:

                        if resource_id.name is not None:
                            dump.add_line('Name: [%s]'
                                    % resource_id.name, 0x06)
                        else:
                            dump.add_line('Id: [0x%X]'
                                    % resource_id.struct.Id, 0x06)

                        dump.add_lines(resource_id.struct.dump(), 0x06)
        ...
        get_sublang_name_for_lang(resource_lang.data.lang,
        resource_lang.data.sublang)), 8)
                                    dump.add_lines(resource_lang.struct.dump(),
        0x0a)
                                    dump.add_lines(resource_lang.data.struct.dump(),
        0x0c)
                            if hasattr(resource_id.directory, 'strings'
                                    ) and resource_id.directory.strings:
                                dump.add_line('[STRINGS]', 0x0a)
                                for (idx, res_string) in \
                                    resource_id.directory.strings.items():
                                    dump.add_line('%6d: %s' % (idx,
        convert_to_printable(res_string)), 0x0c)
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_DEBUG'):
            ...
            for dbg in self.DIRECTORY_ENTRY_DEBUG:
                ...
                try:
                    dump.add_line('Type: '
                                  + DEBUG_TYPE[dbg.struct.Type])
                except KeyError:
                    dump.add_line('Type: 0x%x(Unknown)'
                                  % dbg.struct.Type)
                dump.add_newline()
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_BASERELOC'):
            ...
            for base_reloc in self.DIRECTORY_ENTRY_BASERELOC:
                ...
                for reloc in base_reloc.entries:
                    try:
                        dump.add_line('%08Xh %s' % (reloc.rva,
    ...
    def dump_dict(self, dump=None):
        ...
        if hasattr(self, 'OPTIONAL_HEADER') and self.OPTIONAL_HEADER \
            is not None:
            dump_dict['OPTIONAL_HEADER'] = \
                self.OPTIONAL_HEADER.dump_dict()
        ...
        if hasattr(self, 'VS_VERSIONINFO'):
            ...
            dump_dict['Version Information'
                      ].append(self.VS_VERSIONINFO.dump_dict())

            if hasattr(self, 'VS_FIXEDFILEINFO'):
                dump_dict['Version Information'
                          ].append(self.VS_FIXEDFILEINFO.dump_dict())
            ...
            if hasattr(self, 'FileInfo'):
                ...
                for entry in self.FileInfo:
                    ...
                    if hasattr(entry, 'StringTable'):
                        ...
                        for st_entry in entry.StringTable:
                            [fileinfo_list.append(line) for line in
                             st_entry.dump_dict()]
                            stringtable_dict['LangID'] = st_entry.LangID
                            for str_entry in st_entry.entries.items():
                                stringtable_dict[convert_to_printable(str_entry[0])] = \
                                    convert_to_printable(str_entry[1])
                        ...
                        for var_entry in entry.Var:
                            ...
                            if hasattr(var_entry, 'entry'):
                                [fileinfo_list.append(line) for line in
                                 var_entry.dump_dict()]
                                var_dict[convert_to_printable(var_entry.entry.keys()[0])] = \
                                    var_entry.entry.values()[0]
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_EXPORT'):
            ...
            dump_dict['Exported symbols'
                      ].append(self.DIRECTORY_ENTRY_EXPORT.struct.dump_dict())
            for export in self.DIRECTORY_ENTRY_EXPORT.symbols:
                export_dict = dict()
                if export.address is not None:
                    export_dict.update({'Ordinal': export.ordinal,
                            'RVA': export.address, 'Name': export.name})
                    if export.forwarder:
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_RESOURCE'):
            ...
            dump_dict['Resource directory'
                      ].append(self.DIRECTORY_ENTRY_RESOURCE.struct.dump_dict())
                        ...
                        if hasattr(resource_id, 'directory'):
                            ...
                            for resource_lang in \
                                ...
                                if hasattr(resource_lang, 'data'):
                                    ...
                                    resource_lang_dict['LANG'] = \
    resource_lang.data.lang
                                    resource_lang_dict['SUBLANG'] = \
    resource_lang.data.sublang
                                    resource_lang_dict['LANG_NAME'] = \
    LANG.get(resource_lang.data.lang, '*unknown*')
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_TLS') \
            ...
            and self.DIRECTORY_ENTRY_TLS.struct:
            dump_dict['TLS'] = \
                self.DIRECTORY_ENTRY_TLS.struct.dump_dict()
    ...
    def get_word_from_offset(self, offset):
        ...
        return self.get_word_from_data(self.__data__[offset:offset
                + 2], 0)
    ...
    def get_qword_from_offset(self, offset):
        ...
        return self.get_qword_from_data(self.__data__[offset:offset
                + 8], 0)
    ...
    def merge_modified_section_data(self):
        ...
        for section in self.sections:
            section_data_start = \
                self.adjust_FileAlignment(section.PointerToRawData,
                    self.OPTIONAL_HEADER.FileAlignment)
            section_data_end = section_data_start \
                + section.SizeOfRawData
            if section_data_start < len(self.__data__) \
                and section_data_end < len(self.__data__):
                self.__data__ = self.__data__[:section_data_start] \
                    + section.get_data() \
    ...
    def relocate_image(self, new_ImageBase):
        ...
        relocation_difference = new_ImageBase \
            ...
            while entry_idx < len(reloc.entries):
                ...
                if entry.type \
                    ...
                    self.set_word_at_rva(entry.rva,
                            ...
                            + relocation_difference >> 16 & 65535)
                    ...
                    self.set_dword_at_rva(entry.rva,
                            self.get_dword_at_rva(entry.rva)
                            + relocation_difference)
                elif entry.type \
                    == RELOCATION_TYPE['IMAGE_REL_BASED_HIGHADJ']:
                    ...
                    self.set_qword_at_rva(entry.rva,
    ...
    def generate_checksum(self):
        ...
        checksum_offset = self.OPTIONAL_HEADER.__file_offset__ + 64  # 64
        ...
        data_len = len(self.__data__) + (4 - remainder) * (remainder
                != 0)
        ...
        for i in xrange(data_len / 4):
            ...
            if i + 1 == data_len / 4 and remainder:
                dword = struct.unpack('I', self.__data__[i * 4:] + '\0'
                        * (4 - remainder))[0]
            else:
                dword = struct.unpack('I', self.__data__[i * 4:i * 4
                        + 4])[0]
    ...
    def is_exe(self):
        ...
        if not self.is_dll() and not self.is_driver() and EXE_flag \
            & self.FILE_HEADER.Characteristics == EXE_flag:
            return True
    ...
    def is_driver(self):
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_IMPORT'):
            ...
            if set(('ntoskrnl.exe', 'hal.dll', 'ndis.sys', 'bootvid.dll'
                   , 'kdcom.dll')).intersection([imp.dll.lower()
                    for imp in self.DIRECTORY_ENTRY_IMPORT]):
                return True
    ...
    def get_overlay_data_start_offset(self):
        ...
        for section in self.sections:
            ...
            if section.PointerToRawData + section.SizeOfRawData \
                > highest_PointerToRawData + highest_SizeOfRawData:
                highest_PointerToRawData = section.PointerToRawData
                highest_SizeOfRawData = section.SizeOfRawData

        if len(self.__data__) > highest_PointerToRawData \
            + highest_SizeOfRawData:
    ...
    def adjust_FileAlignment(self, val, file_alignment):
        ...
        if file_alignment > FILE_ALIGNEMNT_HARDCODED_VALUE:
            ...
            if not power_of_two(file_alignment) \
                and FileAlignment_Warning is False:
                self.__warnings.append('If FileAlignment > 0x200 it should be a power of 2. Value: %x'
                         % file_alignment)
    ...
    def adjust_SectionAlignment(
        ...
        if file_alignment < FILE_ALIGNEMNT_HARDCODED_VALUE:
            if file_alignment != section_alignment \
                and SectionAlignment_Warning is False:
                self.__warnings.append('If FileAlignment(%x) < 0x200 it should equal SectionAlignment(%x)'
                         % (file_alignment, section_alignment))
                SectionAlignment_Warning = True

        if section_alignment < 4096:  # page size
